Shader "Custom/HeatMapHistoryShader1" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 highlightColor = tex2D (_HighlightTex, float2((float) numPings / _Gaze_Length, .5));
			...
			float t = pow(((float)numPings / (float)_Gaze_Length), .2);
			...
			//o.Alpha = c.a;
			o.Alpha = saturate((float)max(0, t));
		}
		ENDCG
	}
	FallBack "Diffuse"
